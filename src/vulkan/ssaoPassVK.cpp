...
bool SSAOPassVK::initialize()
{
    ...
    //SHADER STAGES
    {
        { // difuse
            ...
            VkShaderModule frag_module = m_runtime.m_shader_registry->loadShader( "./shaders/ssao.spv", VK_SHADER_STAGE_FRAGMENT_BIT );
...
void SSAOPassVK::createKernelAndNoise()
{
    // normal-oriented hemisphere
    std::vector<Vector4f> kernel;
    kernel.resize(64);
    ...
    UtilsVK::createBuffer(
        *m_runtime.m_renderer->getDevice(),
        ...
        m_in_ssaoKernelBuffer,
        ssaoKernelMemory
    );

    void* data;
    vkMapMemory(m_runtime.m_renderer->getDevice()->getLogicalDevice(), ssaoKernelMemory, 0,
        kernel.size() * sizeof(Vector4f), 0, &data);

    memcpy(data, kernel.data(), kernel.size() * sizeof(Vector4f));

    vkUnmapMemory(m_runtime.m_renderer->getDevice()->getLogicalDevice(), ssaoKernelMemory);

    // random kernel rotations with noise
    std::vector<Vector4f> noise(16);
    for (int i = 0; i < 16; i++)
    {
        noise[i] = Vector4f(
...
void SSAOPassVK::createDescriptorLayout()
{
    ...
    layout_bindings[ 1 ].descriptorType               = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER; // textura de posicion + profundidad
    layout_bindings[ 1 ].stageFlags                   = VK_SHADER_STAGE_FRAGMENT_BIT;

    layout_bindings[ 2 ] = {};
    layout_bindings[ 2 ].binding                      = 2;
    layout_bindings[ 2 ].descriptorCount              = 1;
    layout_bindings[ 2 ].descriptorType               = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER; // textura de normales
    layout_bindings[ 2 ].stageFlags                   = VK_SHADER_STAGE_FRAGMENT_BIT;

    layout_bindings[ 3 ] = {};
    layout_bindings[ 3 ].binding                      = 3;
    layout_bindings[ 3 ].descriptorCount              = 1;
    layout_bindings[ 3 ].descriptorType               = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER; // textura de numeros aleatorios
    layout_bindings[ 3 ].stageFlags                   = VK_SHADER_STAGE_FRAGMENT_BIT;

    layout_bindings[ 4 ] = {};
    layout_bindings[ 4 ].binding                      = 4;
    layout_bindings[ 4 ].descriptorCount              = 1;
    layout_bindings[ 4 ].descriptorType               = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER; // buffer de kernels
...
void SSAOPassVK::createDescriptors()
{
    ...
    {   
        ...
        image_infos[ 2 ].sampler     = m_in_noiseImage.m_sampler;
        image_infos[ 2 ].imageView   = m_in_noiseImage.m_image_view;
        image_infos[ 2 ].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

        // kernels buffer info
        VkDescriptorBufferInfo kernelBufferInfo{};
        kernelBufferInfo.buffer = m_in_ssaoKernelBuffer;
        kernelBufferInfo.offset = 0;
        kernelBufferInfo.range = sizeof(Vector4f) * 64;

        // write the 5 descriptor sets
        ...
        set_write[ 4 ].pBufferInfo       = &kernelBufferInfo;
